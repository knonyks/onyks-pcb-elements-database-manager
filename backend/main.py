...
@app.post("/manufacturers/create", response_model = schemas.ManufacturerCreateResponse, status_code = status.HTTP_201_CREATED)
...
@app.post("/suppliers/create", response_model = schemas.SupplierCreateResponse, status_code = status.HTTP_201_CREATED)
def write_suppliers_create(supplier: schemas.SupplierCreate, db: Session = Depends(get_db)):
...
@app.get("/manufacturers/", response_model=schemas.DatabaseListResponse[schemas.Manufacturer])
def read_manufacturers(params: schemas.DatabaseList = Depends(), db: Session = Depends(get_db)):
    items, next_cursor, has_more, total_count = crud.get_sorted_manufacturers_list(
        db=db, 
...
@app.get("/suppliers/", response_model=schemas.DatabaseListResponse[schemas.Supplier])
def read_suppliers(params: schemas.DatabaseList = Depends(), db: Session = Depends(get_db)):
    items, next_cursor, has_more, total_count = crud.get_sorted_suppliers_list(
        db=db, 
        cursor_str=params.cursor, 
        limit=params.limit
    )
