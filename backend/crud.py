...
def get_sorted_manufacturers_list(db: Session, cursor_str: str = None, limit: int = 20):
    ...
    if items:
        last_item = items[-1]
        print(last_item.created_at)
        next_cursor = utils.encode_cursor({"name" : last_item.name, "id" : last_item.id, "created_at" : last_item.created_at.isoformat()})

    return items, next_cursor, has_more, total_count

def get_sorted_suppliers_list(db: Session, cursor_str: str = None, limit: int = 20):
